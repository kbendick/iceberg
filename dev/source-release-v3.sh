...
# TODO - Does the above matter now that we're using the .gitattributes file properly?
#      - Overall, the idea was to be less conservative to be sure we picked up all files.
...
git archive $release_hash--worktree-attributes --prefix $tagrc/ -o $tarball $tagrc
